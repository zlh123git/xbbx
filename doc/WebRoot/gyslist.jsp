	...
	$(function(){
		  ...
		  function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/gysmana.action?index=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/gysmana.action?index="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/gysmana.action?index="+totlePage;
	            submit();
	        }
		}
		...
		function modify(id){
			window.location.href="<%=path %>/gysmodifybefore.action?id="+id;
		}
		function bmyg(id){
			window.location.href="<%=path %>/bmgysmana.action?bmid="+id;
		}
		function del(id){
			window.location.href="<%=path %>/gysdel.action?id="+id;
		}
...
<body>
<form id="memberForm" name="memberForm" action="<%=path %>/gysmana.action">
<div id="contentWrap">
	<!--表格控件 -->
