	...
	$(function(){
		  ...
		  function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/goodsxsmana.action?index=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/goodsxsmana.action?index="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/goodsxsmana.action?index="+totlePage;
	            submit();
	        }
		}
		...
		function modify(id){
			window.location.href="<%=path %>/goodsxsadd.jsp?id="+id+"&cgid=<%=cgid%>";
		}
		function del(id){
