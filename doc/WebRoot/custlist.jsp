	...
	$(function(){
		  ...
		  function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/custmana.action?index=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/custmana.action?index="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/custmana.action?index="+totlePage;
	            submit();
	        }
		}
		...
		function modify(id){
			window.location.href="<%=path %>/custmodifybefore.action?id="+id;
		}
		function modify2(id){
			window.location.href="<%=path %>/xsmoremana2222.action?id="+id;
		}
		function bmyg(id){
			window.location.href="<%=path %>/bmcustmana.action?bmid="+id;
		}
		function del(id){
			window.location.href="<%=path %>/custdel.action?id="+id;
		}
		function cz(id){
			window.location.href="<%=path %>/czadd.jsp?id="+id;
		}
	function search(){
...
<body>
<form id="memberForm" name="memberForm" action="<%=path %>/custmana.action">
<div id="contentWrap">
	<!--表格控件 -->
          ...
          <tr>
            ...
            <td><%=bean.getNo() %></td>
             <td><%=bean.getRealname() %></td>
               <td><%=bean.getMoneys() %></td>
                 <td><%=bean.getJf() %></td>
                   <td><%=bean.getTypes() %></td>
              <td><%=bean.getTel() %></td>
              <td><%=bean.getAddress() %></td>
              <td> <a onclick="modify2(<%=bean.getId()%>)"><img src="images/icon/edit2.png" width="16" height="16" /></a> 
            </td>
            ...
            <td> 
             <a onclick="cz(<%=bean.getId()%>)"><img src="images/icon/del.png" width="16" height="16" /></a> 
            </td>
          </tr>
