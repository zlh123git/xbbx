	...
	$(function(){
		  ...
		  function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/goodsmana.action?index=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/goodsmana.action?index="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/goodsmana.action?index="+totlePage;
	            submit();
	        }
		}
      ...
      <table>
        ...
        <%
			for(int i = 0;i<list.size();i++){
				...
				String pic=path+"/upfile/"+bean.getPic();
			%>
          <tr>
            <td><%=bean.getId() %></td>
            <td><%=bean.getNo() %></td>
