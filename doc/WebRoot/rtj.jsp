...
<body>
<form id="memberForm" name="memberForm" action="<%=path %>/rtj.action">
<div id="contentWrap">
	<!--表格控件 -->
  <div id="widget table-widget">
    <div class="pageTitle">销售统计</div>
      ...
      <table>
        ...
        <%
			for(int i = 0;i<list.size();i++){
				...
				double price=Double.parseDouble(ValueBean.getJjpriceByGId(bean.getGid()));
				double sjprice=bean.getSj();
				double yl=(sjprice-price)*bean.getSl();
				System.out.println("---------------------"+bean.getGid());
			%>
          <tr>
                  <td><%=ValueBean.getnamesByGId(bean.getGid()) %></td>
                  <td><%=ValueBean.getnoByGId(bean.getGid()) %></td>
                  <td><%=ValueBean.getpriceByGId(bean.getGid()) %></td>
                   <td><%=bean.getSj() %></td>
             <td><%=bean.getPrice() %></td>
              <td><%=yl %></td>
               <td><%=bean.getSl() %></td>
