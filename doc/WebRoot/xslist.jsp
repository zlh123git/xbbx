	...
	$(function(){
		  ...
		  function frist()
		{
		 	with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/xsmana.action?index=1";
	            submit();
	        }
		}
		...
		function back()
		{
		    ...
		    if(pageNo<2){
		 	   ...
		 	   pageNo=Number(pageNo)-1;
				 ...
				 with (document.getElementById("memberForm")) {
			            method = "post";
			             action = "<%=path %>/xsmana.action?index="+pageNo;
			            submit();
			        }
		}
		...
		{ 
		   var totlePage='<%=totlePage%>';
			with (document.getElementById("memberForm")) {
	            method = "post";
	            action = "<%=path %>/xsmana.action?index="+totlePage;
	            submit();
	        }
		}
		...
		function view(id){
			window.location.href="<%=path %>/xsmoremana.action?cgid="+id;
		}
		function cgadd(){
			window.location.href="<%=path %>/xsaddbf.action";
		}
</script>
<style type="text/css">
body {
...
<div id="contentWrap">
  ...
  <div id="widget table-widget">
    <div class="pageTitle">我的销售</div>
    <div class="pageColumn">
      <div class="pageButton"><a href="#" onclick="cgadd()">新增销售</a>  </div>
      <table>
        <thead>
          <th width="">id</th>
          <th width="">订单号</th>
        ...
        <%
			for(int i = 0;i<list.size();i++){
				Xs bean = (Xs)list.get(i);
			%>
          <tr>
            <td><%=bean.getId() %></td>
            <td><%=bean.getNo() %></td>
             <td><%=bean.getCustnames() %></td>
             <td><%=bean.getPrice() %></td>
               <td><%=bean.getTimes() %></td>
